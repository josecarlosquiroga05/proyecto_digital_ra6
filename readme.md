Proyecto RA6 · Transformación Digital
1) Empresa
Nombre: FreshBite
Sector: Restauración (comida saludable)
Situación actual:
Restaurante local con pedidos presenciales y telefónicos. Control manual de inventario y sin presencia digital relevante. No dispone de web ni sistema de gestión integrado.
2) Objetivos
Digitalizar el proceso de pedidos mediante una plataforma online.
Automatizar la gestión de inventario y proveedores.
Mejorar la experiencia del cliente con seguimiento de pedidos y fidelización.
3) Áreas
Ventas: Gestión de pedidos online y en local.
Logística: Control de inventario y abastecimiento.
Marketing: Promoción en redes sociales y fidelización.

Relación:
Ventas genera datos que alimentan logística (stock) y marketing (hábitos del cliente). Logística asegura disponibilidad para ventas. Marketing impulsa la demanda.
4) Tecnologías
Cloud computing: Para alojar la web y base de datos.
Base de datos (SQL/NoSQL): Para clientes, pedidos e inventario.
Aplicación web/móvil: Para pedidos online.
IA básica: Recomendación de platos según historial.
5) Datos
Datos que maneja:
Clientes (nombre, preferencias, historial)
Pedidos
Inventario
Proveedores
Información obtenida:
Productos más vendidos
Horas pico
Preferencias de clientes
Predicción de demanda
6) Seguridad
Riesgos:
Robo de datos de clientes
Ataques a la web (DDoS o hacking)
Medidas de protección:
Cifrado de datos (HTTPS, SSL)
Copias de seguridad y autenticación segura (2FA)
7) RRHH
Nuevas competencias digitales:
Uso de software de gestión
Marketing digital
Análisis de datos básico
Cambios organizativos:
Creación de rol de gestor digital
Formación del personal en herramientas tecnológicas
10) UML
Casos de uso:
Cliente: hacer pedido, registrarse, pagar
Empleado: gestionar pedidos, actualizar stock
Administrador: ver informes, gestionar sistema
Clases:
Cliente (id, nombre, email)
Pedido (id, fecha, total)
Producto (id, nombre, precio)
Inventario (cantidad, producto)
Usuario (rol)

11) Conclusión

La digitalización de FreshBite permite optimizar procesos, mejorar la experiencia del cliente y aumentar la rentabilidad. Adoptar tecnologías digitales no solo es una ventaja competitiva, sino una necesidad para adaptarse al mercado actual.
